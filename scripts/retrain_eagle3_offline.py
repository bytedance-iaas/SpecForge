...
def parse_args():
    ...
    parser.add_argument("--strict-load", action="store_true", help="Strict loading of checkpoint state dict")
...
def load_checkpoint_state(checkpoint_path, strict=False):
    """Load training state from checkpoint"""
    try:
        training_state_path = os.path.join(checkpoint_path, "training_state.pt")
        if os.path.exists(training_state_path):
            state = torch.load(training_state_path, map_location="cpu")
            print_on_rank0(f"Loaded training state from {training_state_path}")
            return state
        else:
            if strict:
                raise FileNotFoundError(f"Training state not found at {training_state_path}")
            print_on_rank0(f"Warning: Training state not found at {training_state_path}")
            return None
    except Exception as e:
        if strict:
            raise e
        print_on_rank0(f"Warning: Failed to load training state: {e}")
        return None
...
def main():
    ...
    if args.resume_epoch is not None:
        ...
        epoch_checkpoint = os.path.join(args.output_dir, f"epoch_{args.resume_epoch}")
        if os.path.exists(epoch_checkpoint):
            draft_model_last_checkpoint = epoch_checkpoint
            start_epoch = args.resume_epoch + 1
            training_state = load_checkpoint_state(epoch_checkpoint, strict=args.strict_load)
            print_on_rank0(f"Resuming from epoch {args.resume_epoch}")
        else:
            raise FileNotFoundError(f"Checkpoint for epoch {args.resume_epoch} not found at {epoch_checkpoint}")
    elif args.resume and os.path.isdir(args.output_dir):
        # Resume from latest checkpoint
        draft_model_last_checkpoint = get_last_checkpoint(args.output_dir)
        if draft_model_last_checkpoint:
            # Extract epoch number from checkpoint path
            epoch_name = os.path.basename(draft_model_last_checkpoint)
            if epoch_name.startswith("epoch_"):
                try:
                    last_epoch = int(epoch_name.split("_")[1])
                    start_epoch = last_epoch + 1
                    training_state = load_checkpoint_state(draft_model_last_checkpoint, strict=args.strict_load)
                    print_on_rank0(f"Resuming from epoch {last_epoch}, will start from epoch {start_epoch}")
                except (ValueError, IndexError):
                    print_on_rank0(f"Warning: Could not parse epoch from {epoch_name}, starting from epoch 0")
            else:
                print_on_rank0(f"Warning: Unexpected checkpoint format {epoch_name}, starting from epoch 0")
        else:
    ...
    if training_state is not None:
        try:
            ...
            print_on_rank0("Loaded optimizer state from checkpoint")
            # Calculate global_step from start_epoch
            steps_per_epoch = math.ceil(len(train_dataloader) / args.draft_accumulation_steps)
            global_step = start_epoch * steps_per_epoch
            print_on_rank0(f"Calculated global_step: {global_step}")
        except Exception as e:
            print_on_rank0(f"Warning: Failed to load optimizer state: {e}")
            if args.strict_load:
                raise e
    ...
    for epoch in range(start_epoch, args.num_epochs):
        print_on_rank0(f"Starting epoch {epoch}/{args.num_epochs}")
